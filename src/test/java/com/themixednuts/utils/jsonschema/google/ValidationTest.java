 ...
 * schemas validate data correctly per Google AI API specification. For schema
...
class ValidationTest {
  ...
  void number_validValue_passes() {
    var schema = SchemaBuilder.number().minimum(0.0).maximum(100.0).build();
  ...
  void object_validProperties_passes() {
    ...
    var errors = validate(schema, mapper.createObjectNode().put("name", "John").put("age", 30));
  ...
  void object_tooManyProperties_fails() {
    ...
    var errors = validate(schema, mapper.createObjectNode().put("a", 1).put("b", 2).put("c", 3));
  ...
  void object_invalidPropertyValue_fails() {
    var schema = SchemaBuilder.object().property("age", SchemaBuilder.integer().minimum(0)).build();
  ...
  void anyOf_matchesBoth_passes() {
    ...
    var schema =
        SchemaBuilder.string()
            .anyOf(SchemaBuilder.string().minLength(3), SchemaBuilder.string().minLength(5))
            .build();
  ...
  void anyOf_withNullable_allowsNull() {
    var schema =
        SchemaBuilder.anyOf(SchemaBuilder.string().minLength(1), SchemaBuilder.integer().minimum(0))
            .nullable(true)
            .build();
  ...
  void nested_objectWithNestedObject_invalidValue_fails() {
    var schema =
        SchemaBuilder.object()
            .property(
                "user", SchemaBuilder.object().property("age", SchemaBuilder.integer().minimum(0)))
            .build();

    var errors =
        validate(
            schema,
            mapper.createObjectNode().set("user", mapper.createObjectNode().put("age", -5)));
  ...
  void complex_userSchema_valid_passes() {
    var schema =
        SchemaBuilder.object()
            .property("id", SchemaBuilder.integer().format(IntegerFormatType.INT64).minimum(1))
            .property("name", SchemaBuilder.string().minLength(1).maxLength(100))
            .property("email", SchemaBuilder.string().pattern("^[^@]+@[^@]+\\.[^@]+$"))
            .property("status", SchemaBuilder.string().enumValues("active", "inactive", "pending"))
            .property(
                "tags",
                SchemaBuilder.array().items(SchemaBuilder.string()).minItems(0).maxItems(10))
            .requiredProperty("id")
            .requiredProperty("name")
    ...
    var schema =
        SchemaBuilder.object()
            .property(
                "value",
                SchemaBuilder.anyOf(
                    SchemaBuilder.string().pattern("^[A-Z]"), SchemaBuilder.integer().minimum(100)))
            .build();
  ...
  void emptyArray_passesMinItemsZero() {
    var schema = SchemaBuilder.array().items(SchemaBuilder.string()).minItems(0).build();
  ...
  void largeNumbers_validateCorrectly() {
    var schema = SchemaBuilder.integer().minimum(Long.MIN_VALUE).maximum(Long.MAX_VALUE).build();
