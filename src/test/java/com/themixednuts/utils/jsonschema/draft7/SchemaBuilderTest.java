...
/**
 * Tests for Draft 7 SchemaBuilder API and JSON serialization.
 * These tests verify that schemas are built correctly and generate the right
 * JSON structure.
 * For data validation tests, see ValidationTest.java.
 */
class SchemaBuilderTest {
    ...
    void allOf_staticMethod_noBaseType() {
        var schema = SchemaBuilder.allOf(
    ...
    void not_staticMethod_noBaseType() {
        var schema = SchemaBuilder.not(SchemaBuilder.string().pattern("password"))
                .build();
    ...
    void anyOf_emptyArray_throwsException() {
        assertThrows(IllegalArgumentException.class, () -> SchemaBuilder.string()
                .anyOf(new com.themixednuts.utils.jsonschema.IBuildableSchemaType[0]).build());
    }
