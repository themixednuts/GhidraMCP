 ...
 * generated schemas validate data correctly per JSON Schema Draft 7 specification. For schema
 * building/serialization tests, see SchemaBuilderTest.java.
 */
class ValidationTest {
  ...
  void string_validValue_passes() {
    var schema = SchemaBuilder.string().minLength(3).maxLength(10).pattern("^[a-zA-Z]+$").build();
  ...
  void array_validSize_passes() {
    var schema =
        SchemaBuilder.array().items(SchemaBuilder.string()).minItems(1).maxItems(5).build();
  ...
  void array_uniqueItems_noDuplicates_passes() {
    var schema = SchemaBuilder.array().items(SchemaBuilder.string()).uniqueItems(true).build();
  ...
  void array_itemSchema_invalidItem_fails() {
    var schema = SchemaBuilder.array().items(SchemaBuilder.integer().minimum(0)).build();
  ...
  void object_validProperties_passes() {
    ...
    assertTrue(isValid(schema, mapper.createObjectNode().put("name", "John").put("age", 30)));
  }
  ...
  void object_tooManyProperties_fails() {
    ...
    assertFalse(isValid(schema, mapper.createObjectNode().put("a", 1).put("b", 2).put("c", 3)));
  }
  ...
  void object_additionalProperties_false_extraProperty_fails() {
    ...
    assertFalse(
        isValid(schema, mapper.createObjectNode().put("name", "test").put("extra", "not allowed")));
  }
  ...
  void object_additionalProperties_schema_validExtra_passes() {
    ...
    assertTrue(isValid(schema, mapper.createObjectNode().put("name", "test").put("count", 5)));
  }
  ...
  void object_additionalProperties_schema_invalidExtra_fails() {
    ...
    assertFalse(
        isValid(
            schema, mapper.createObjectNode().put("name", "test").put("count", "not an integer")));
  }
  ...
  void oneOf_matchesMultiple_fails() {
    var schema =
        SchemaBuilder.string()
            .oneOf(SchemaBuilder.string().minLength(3), SchemaBuilder.string().minLength(5))
            .build();
  ...
  void anyOf_matchesNone_fails() {
    var schema =
        SchemaBuilder.anyOf(
                SchemaBuilder.string().minLength(10), SchemaBuilder.string().pattern("^admin"))
            .build();
  ...
  void ifThen_conditionTrue_thenApplies() {
    ...
    assertTrue(
        isValid(schema, mapper.createObjectNode().put("type", "admin").put("password", "secret")));

    // Admin without password - fails
    assertFalse(isValid(schema, mapper.createObjectNode().put("type", "admin")));
  }
  ...
  void ifThen_conditionFalse_thenIgnored() {
    ...
    assertTrue(isValid(schema, mapper.createObjectNode().put("type", "user")));
  }
  ...
  void ifThenElse_conditionTrue_thenApplies() {
    var schema =
        SchemaBuilder.object()
            .property("type", SchemaBuilder.string())
            .ifThenElse(
                SchemaBuilder.object().property("type", SchemaBuilder.string().constValue("short")),
                SchemaBuilder.object().property("value", SchemaBuilder.string().maxLength(10)),
                SchemaBuilder.object().property("value", SchemaBuilder.string().minLength(20)))
            .build();

    // Type "short" with short value - passes
    assertTrue(
        isValid(schema, mapper.createObjectNode().put("type", "short").put("value", "hello")));
  ...
  void ifThenElse_conditionFalse_elseApplies() {
    ...
    assertFalse(
        isValid(schema, mapper.createObjectNode().put("type", "long").put("value", "short")));
  }
    ...
    assertFalse(
        isValid(
            ...
            mapper
                ...
                .set(
                    "users",
                    mapper.createArrayNode().add(mapper.createObjectNode().put("id", -1)))));
  }
}
