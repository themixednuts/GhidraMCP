 ...
 * are built correctly and generate the right JSON per Google AI API specification.
 *
 * <p>Note: Google AI API format is NOT standard JSON Schema - it has specific requirements: -
 * Numeric constraints (minLength, maxItems, etc.) as STRINGS (int64 format) - Min/max bounds
 * (minimum, maximum) as NUMBERS - nullable boolean on all types - example (singular value) -
...
class SchemaBuilderTest {
  ...
  void string_enum_autoSetsFormatEnum() {
    var schema = SchemaBuilder.string().enumValues("EAST", "NORTH", "SOUTH", "WEST").build();
  ...
  void array_withConstraints_serializesCorrectly() {
    var schema =
        SchemaBuilder.array().items(SchemaBuilder.string()).minItems(1).maxItems(10).build();
  ...
  void anyOf_withBaseType_hasType() {
    ...
    var schema =
        SchemaBuilder.string()
            .anyOf(SchemaBuilder.string().minLength(1), SchemaBuilder.string().maxLength(100))
            .build();
  ...
  void complexSchema_allGoogleFeatures() {
    var schema =
        SchemaBuilder.object()
            ...
            .property(
                "status",
                SchemaBuilder.string().enumValues("active", "inactive")) // Auto-sets format:enum
            .requiredProperty("id")
  ...
  void string_emptyEnumArray_throwsException() {
    assertThrows(IllegalArgumentException.class, () -> SchemaBuilder.string().enumValues());
  }

  @Test
  void string_nullPattern_throwsException() {
    assertThrows(NullPointerException.class, () -> SchemaBuilder.string().pattern(null));
  }
