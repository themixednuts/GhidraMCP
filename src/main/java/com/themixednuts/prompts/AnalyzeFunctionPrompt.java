...
/**
 * Prompt for analyzing a function in detail.
 * Provides decompiled code and context for comprehensive analysis.
 */
@GhidraMcpPrompt(
    name = "analyze_function",
    title = "Analyze Function",
    description = "Analyze a function in detail including decompiled code, references, and potential issues. " +
...
public class AnalyzeFunctionPrompt extends BaseMcpPrompt {
    ...
    public List<PromptArgument> getArguments() {
        return List.of(
                ...
                new PromptArgument("function_address", "Entry point address of the function to analyze", true),
                new PromptArgument("analysis_focus", "Specific focus area: security, performance, logic, or general", false)
        );
    }
    ...
    public Mono<GetPromptResult> generate(McpTransportContext context, Map<String, Object> arguments, PluginTool tool) {
        return Mono.fromCallable(() -> {
            ...
            try {
                ...
                DecompileResults results = decompiler.decompileFunction(function, DECOMPILE_TIMEOUT_SECONDS, TaskMonitor.DUMMY);
                ...
                promptText.append("- **Signature**: ").append(function.getPrototypeString(false, false)).append("\n");
                promptText.append("- **Calling Convention**: ").append(function.getCallingConventionName()).append("\n");
                promptText.append("- **Parameter Count**: ").append(function.getParameterCount()).append("\n");
                ...
                if (results.decompileCompleted() && results.getDecompiledFunction() != null) {
                    ...
                    promptText.append(results.getErrorMessage() != null ? results.getErrorMessage() : "Unknown error");
                }
                promptText.append("\n```\n\n");
                
                // Add references
                promptText.append("## References\n");
                ReferenceIterator refIter = program.getReferenceManager().getReferencesTo(function.getEntryPoint());
                int refCount = 0;
                ...
                while (refIter.hasNext() && refCount < 10) {
                    ...
                    if (caller != null) {
                        promptText.append("- ").append(caller.getName())
                                .append(" (").append(ref.getFromAddress()).append(")\n");
                    }
                    refCount++;
                }
                if (refCount == 0) {
