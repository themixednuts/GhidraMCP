 ...
 * Provides reverse engineering workflow prompts for AI assistants.
 * 
 * <p>Implementations should be annotated with @GhidraMcpPrompt and registered
 * via ServiceLoader in META-INF/services/com.themixednuts.prompts.BaseMcpPrompt.
 */
public abstract class BaseMcpPrompt {
     ...
     */
    public abstract Mono<GetPromptResult> generate(McpTransportContext context, Map<String, Object> arguments, PluginTool tool);
    ...
    public AsyncPromptSpecification toPromptSpecification(PluginTool tool) {
        ...
        return new AsyncPromptSpecification(
                prompt,
                (ctx, request) -> handleGetPrompt(ctx, request, tool)
        );
    }
     ...
     */
    protected Mono<GetPromptResult> handleGetPrompt(McpTransportContext ctx, GetPromptRequest request, PluginTool tool) {
        return generate(ctx, request.arguments(), tool)
                .onErrorResume(t -> {
                    String errorMsg = t instanceof GhidraMcpException ? 
                            t.getMessage() : "Error generating prompt: " + t.getMessage();
     ...
     */
    protected String getRequiredArgument(Map<String, Object> args, String name) throws GhidraMcpException {
