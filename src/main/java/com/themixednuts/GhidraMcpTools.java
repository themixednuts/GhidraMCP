 ...
 * providing MCP tool specifications.
 *
 * <p>This class implements the {@link IGhidraMcpToolProvider} service interface and is registered
 * by {@link GhidraMcpPlugin}.
 */
public class GhidraMcpTools implements IGhidraMcpToolProvider {
  ...
  /**
   * Discovers available {@link BaseMcpTool} implementations using {@link ServiceLoader}, filters
   * them based on whether they are enabled in the Ghidra Tool Options, and generates their {@link
        ...
        .map(
            provider -> {
              try {
                ...
                Msg.error(
                    GhidraMcpTools.class, "Error getting specification for tool: " + className, e);
                return null;
              }
            })
  ...
  /**
   * Static utility method to register Ghidra Tool Options for enabling/disabling individual MCP
   * tools discovered via {@link ServiceLoader}.
   *
              ...
              try {
                ...
                if (toolAnnotation == null) {
                  Msg.warn(
                      ...
                      "Tool class "
                          + toolClass.getSimpleName()
                          + " is missing the @GhidraMcpTool annotation. Skipping option"
                          + " registration.");
                  return;
                }
                ...
                Msg.error(
                    GhidraMcpTools.class,
                    "Security exception accessing annotation for tool: "
                        + toolClass.getSimpleName(),
                    e);
              } catch (Exception e) {
                Msg.error(
