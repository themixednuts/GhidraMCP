 ...
 *
 * <p>This builder implements the Google AI API schema specification, which is a subset of OpenAPI
 * 3.0 schema. It includes Google-specific extensions like:
 *
 * <ul>
 *   <li>nullable (boolean) - Indicates if value may be null
 *   <li>example (value) - Example value for documentation
 *   <li>propertyOrdering (array) - Google-specific property order control
 * </ul>
 *
 * <p>Example Usage:
 *
...
public class SchemaBuilder {
  ...
  /**
   * Private constructor for creating a SchemaBuilder instance. Only used internally; clients should
   ...
   *
   * <p>String schemas support minLength, maxLength, pattern, format, and enum. Note: When
   * enumValues() is called, format is automatically set to "enum" per Google spec.
   */
  public interface IStringSchemaBuilder
      extends IBuildableSchemaType,
   ...
   *
   * <p>Number schemas support minimum, maximum, and format constraints.
   */
  public interface INumberSchemaBuilder
      extends IBuildableSchemaType,
   ...
   *
   * <p>Integer schemas support minimum, maximum, and format constraints.
   */
  public interface IIntegerSchemaBuilder
      extends IBuildableSchemaType,
   ...
   *
   * <p>Boolean schemas only support common metadata, nullable, example, and anyOf.
   */
  public interface IBooleanSchemaBuilder
      extends IBuildableSchemaType,
   ...
   *
   * <p>Null schemas only support common metadata, nullable, example, and anyOf.
   */
  public interface INullSchemaBuilder
      extends IBuildableSchemaType,
   ...
   *
   * <p>Array schemas support items, minItems, and maxItems constraints.
   */
   ...
   *
   * <p>Object schemas support properties, required fields, min/maxProperties, and the
   * Google-specific propertyOrdering field.
   */
  ...
  /**
   * Creates a schema with ONLY anyOf constraint and NO base type. Use this for union types where
   * data can match one or more of several different types.
   *
   * <p>Note: Google AI API only supports anyOf (not allOf/oneOf/not per the specification).
   *
   * <p>Example: String OR Integer
   *
   * <pre>{@code
