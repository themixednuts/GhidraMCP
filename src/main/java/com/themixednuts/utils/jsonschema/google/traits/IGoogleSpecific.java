...
/**
 * Capability interface for Google AI API-specific schema fields (nullable, example). These are
 * OpenAPI/Google extensions not present in pure JSON Schema.
 *
...
public interface IGoogleSpecific<SELF> {
  ...
  /**
   * Specifies if this schema can be null. This is an OpenAPI extension supported by Google AI API.
   *
  ...
  /**
   * Sets an example value for this schema. According to Google spec: "Will only be populated when
   * the object is the root." However, this method is available on all builders for flexibility.
   *
