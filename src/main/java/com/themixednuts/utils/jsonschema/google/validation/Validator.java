 ...
 * 
 * <p>
 * Validates JSON data against Google AI API schema format, which differs from
 * standard
 * JSON Schema:
 * </p>
 * <ul>
 * <li>Numeric constraints (minLength, maxItems, etc.) are STRINGS with int64
 * format</li>
 * <li>Min/max bounds (minimum, maximum) are NUMBERS</li>
 * <li>nullable boolean indicates if value may be null</li>
 * <li>Only anyOf composition keyword (no allOf/oneOf/not)</li>
 * <li>No conditional keywords (no if/then/else)</li>
 * </ul>
 * 
 * <p>
 * Example usage:
...
public class Validator {
    ...
    private static Set<ValidationMessage> validate(ObjectNode schema, JsonNode data, String path) {
        ...
        if (data == null || data.isNull()) {
            ...
            errors.add(ValidationMessage.error(path, "nullable", "Value is null but nullable is not true"));
            return errors;
        }
        ...
        switch (type) {
            ...
            case "null":
                if (!data.isNull()) {
                    errors.add(ValidationMessage.error(path, "type", "Expected null but got " + data.getNodeType()));
                }
                break;
            default:
    ...
    private static void validateString(ObjectNode schema, JsonNode data, String path, Set<ValidationMessage> errors) {
        if (!data.isTextual()) {
            errors.add(ValidationMessage.error(path, "type", "Expected string but got " + data.getNodeType()));
            return;
        }
        ...
        if (schema.has("minLength")) {
            try {
                ...
                if (value.length() < minLength) {
                    errors.add(ValidationMessage.error(path, "minLength",
                            String.format("String length %d is less than minLength %d", value.length(), minLength)));
                }
        ...
        if (schema.has("maxLength")) {
            try {
                ...
                if (value.length() > maxLength) {
                    errors.add(ValidationMessage.error(path, "maxLength",
                            String.format("String length %d is greater than maxLength %d", value.length(), maxLength)));
                }
            } catch (NumberFormatException e) {
                errors.add(ValidationMessage.error(path, "maxLength",
        ...
        if (schema.has("pattern")) {
            ...
            try {
                ...
                if (!pattern.matcher(value).find()) {
                    errors.add(ValidationMessage.error(path, "pattern",
                            String.format("String '%s' does not match pattern '%s'", value, patternStr)));
                }
            } catch (PatternSyntaxException e) {
                errors.add(ValidationMessage.error(path, "pattern", "Invalid regex pattern: " + patternStr));
            }
        }
        ...
        if (schema.has("enum")) {
            ...
            if (!found) {
                errors.add(ValidationMessage.error(path, "enum",
                        String.format("String '%s' is not in enum values", value)));
            }
        }
    }
    ...
    private static void validateInteger(ObjectNode schema, JsonNode data, String path, Set<ValidationMessage> errors) {
        if (!data.isIntegralNumber()) {
            errors.add(ValidationMessage.error(path, "type", "Expected integer but got " + data.getNodeType()));
            return;
        }
        ...
        if (schema.has("minimum")) {
            ...
            if (value < minimum) {
                errors.add(ValidationMessage.error(path, "minimum",
        ...
        if (schema.has("maximum")) {
            ...
            if (value > maximum) {
                errors.add(ValidationMessage.error(path, "maximum",
    ...
    private static void validateNumber(ObjectNode schema, JsonNode data, String path, Set<ValidationMessage> errors) {
        if (!data.isNumber()) {
            errors.add(ValidationMessage.error(path, "type", "Expected number but got " + data.getNodeType()));
            return;
        }
    ...
    private static void validateBoolean(ObjectNode schema, JsonNode data, String path, Set<ValidationMessage> errors) {
        if (!data.isBoolean()) {
            errors.add(ValidationMessage.error(path, "type", "Expected boolean but got " + data.getNodeType()));
        }
    }

    // ========== Array Validation ==========

    private static void validateArray(ObjectNode schema, JsonNode data, String path, Set<ValidationMessage> errors) {
        if (!data.isArray()) {
            errors.add(ValidationMessage.error(path, "type", "Expected array but got " + data.getNodeType()));
            return;
        }
        ...
        if (schema.has("minItems")) {
            try {
                ...
                if (size < minItems) {
                    errors.add(ValidationMessage.error(path, "minItems",
                            String.format("Array size %d is less than minItems %d", size, minItems)));
                }
            } catch (NumberFormatException e) {
                errors.add(ValidationMessage.error(path, "minItems",
                        "Invalid minItems format: " + schema.get("minItems").asText()));
            }
        }
        ...
        if (schema.has("maxItems")) {
            try {
                ...
                if (size > maxItems) {
                    errors.add(ValidationMessage.error(path, "maxItems",
                            String.format("Array size %d is greater than maxItems %d", size, maxItems)));
                }
            } catch (NumberFormatException e) {
                errors.add(ValidationMessage.error(path, "maxItems",
                        "Invalid maxItems format: " + schema.get("maxItems").asText()));
            }
        }
    ...
    private static void validateObject(ObjectNode schema, JsonNode data, String path, Set<ValidationMessage> errors) {
        if (!data.isObject()) {
            errors.add(ValidationMessage.error(path, "type", "Expected object but got " + data.getNodeType()));
            return;
        }
        ...
        if (schema.has("minProperties")) {
            try {
                ...
                if (propertyCount < minProperties) {
                    errors.add(ValidationMessage.error(path, "minProperties",
                            String.format("Object has %d properties, less than minProperties %d", propertyCount,
                                    minProperties)));
                }
            } catch (NumberFormatException e) {
                errors.add(ValidationMessage.error(path, "minProperties",
        ...
        if (schema.has("maxProperties")) {
            try {
                ...
                if (propertyCount > maxProperties) {
                    errors.add(ValidationMessage.error(path, "maxProperties",
                            String.format("Object has %d properties, greater than maxProperties %d", propertyCount,
                                    maxProperties)));
                }
            } catch (NumberFormatException e) {
                errors.add(ValidationMessage.error(path, "maxProperties",
        ...
        if (schema.has("required")) {
            ...
            for (JsonNode requiredProp : required) {
                ...
                if (!object.has(propName)) {
                    errors.add(ValidationMessage.error(path, "required",
                            String.format("Required property '%s' is missing", propName)));
                }
            }
        }
    ...
    private static Set<ValidationMessage> validateAnyOf(JsonNode anyOfNode, JsonNode data, String path) {
        Set<ValidationMessage> errors = new HashSet<>();
        ...
        if (!matched) {
            errors.add(ValidationMessage.error(path, "anyOf", "Data does not match any of the anyOf schemas"));
        }
