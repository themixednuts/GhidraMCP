...
/**
 * Capability interface for conditional keywords (if/then/else). Draft 7 allows these on any schema
 * type for conditional validation.
 *
 ...
 * @see <a href=
 *     "https://json-schema.org/draft-07/json-schema-validation.html#rfc.section.6.6">Draft 7
 *     Conditionals</a>
 */
public interface IConditionals<SELF> {

  /**
   * Adds conditional validation - if the "if" schema validates, then the "then" schema must also
   * validate.
   *
  ...
  /**
   * Adds conditional validation with else clause - if the "if" schema validates, then the "then"
   * schema must also validate, otherwise the "else" schema must validate.
   *
   * @param ifSchema The condition schema
   * @param thenSchema The schema to apply if condition is true
