...
/**
 * Provides MCP prompt specifications for reverse engineering workflows.
 * Discovers prompts via ServiceLoader from META-INF/services.
 */
public class GhidraMcpPrompts implements IGhidraMcpPromptProvider {
    ...
    public List<AsyncPromptSpecification> getPromptSpecifications() {
        ...
        ServiceLoader.load(BaseMcpPrompt.class).forEach(prompt -> {
            GhidraMcpPrompt annotation = prompt.getClass().getAnnotation(GhidraMcpPrompt.class);
            if (annotation == null) {
                Msg.warn(this, "Prompt " + prompt.getClass().getSimpleName() + 
            ...
            try {
                ...
                Msg.error(this, "Failed to create specification for prompt: " + annotation.name(), e);
            }
        });
