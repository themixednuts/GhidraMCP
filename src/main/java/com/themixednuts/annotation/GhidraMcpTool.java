...
public @interface GhidraMcpTool {
	...
	/**
	 * Human-readable title for the tool (optional, used in UIs).
	 * If empty, the mcpName will be used.
	 */
	String title() default "";

	/**
	 * If true, indicates the tool does not modify any state (read-only operation).
	 * Clients may use this to parallelize read-only operations safely.
	 */
	boolean readOnlyHint() default false;

	/**
	 * If true, indicates the tool may perform destructive operations 
	 * (e.g., deleting functions, symbols, or data types).
	 * Clients should exercise caution and potentially confirm with users.
	 */
	boolean destructiveHint() default false;

	/**
	 * If true, indicates the tool is idempotent - calling it multiple times
	 * with the same arguments produces the same result and has no additional effect.
