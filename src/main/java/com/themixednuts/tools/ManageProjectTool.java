...
@GhidraMcpTool(
    name = "Manage Project",
    description =
        "Project-level operations covering bookmarks, navigation, analysis settings, and program"
            + " metadata.",
...
public class ManageProjectTool extends BaseMcpTool {
  ...
  public JsonSchema schema() {
    ...
    schemaRoot.property(
        ...
        SchemaBuilder.string(mapper)
            .description(
                "Filter for bookmark deletion: matches bookmarks whose comment contains this"
                    + " text."));
  ...
  private Mono<? extends Object> handleCreateBookmark(
    ...
    if (bookmarkType.isBlank()) {
      return buildBlankArgumentError(
          annotation,
          args,
          ARG_BOOKMARK_TYPE,
          ACTION_CREATE_BOOKMARK,
                    ...
                    () -> {
                      ...
                      try {
                        ...
                        bookmarkManager.setBookmark(
                            address, bookmarkType, bookmarkCategory, comment);
                        ...
                        GhidraMcpError error =
                            GhidraMcpError.failed("create bookmark", e.getMessage());
  ...
  private Mono<? extends Object> handleGoToAddress(
      Program program, Map<String, Object> args, PluginTool tool, GhidraMcpTool annotation) {
    String addressStr;
    try {
      addressStr = getRequiredStringArgument(args, ARG_ADDRESS);
    } catch (GhidraMcpException e) {
