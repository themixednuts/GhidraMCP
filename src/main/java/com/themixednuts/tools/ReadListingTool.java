...
public class ReadListingTool extends BaseMcpTool {
    ...
    private PaginatedResult<ListingInfo> listListingInRange(Program program, Address startAddr, Address endAddr,
        ...
        if (codeUnit != null) {
            ...
            } while (codeUnit != null && codeUnit.getMinAddress().compareTo(endAddr) <= 0
        ...
        List<ListingInfo> paginatedItems = allItems.stream()
                .dropWhile(item -> {
        ...
        List<ListingInfo> results = paginatedItems.subList(0, Math.min(paginatedItems.size(), maxLines));
