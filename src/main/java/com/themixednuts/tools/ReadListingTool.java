...
public class ReadListingTool extends BaseMcpTool {
  ...
  private Mono<? extends Object> handleDefaultStart(
      Program program, Map<String, Object> args, GhidraMcpTool annotation) {
    return Mono.fromCallable(
        () -> {
          ...
          if (firstBlock == null) {
            throw new GhidraMcpException(
                GhidraMcpError.resourceNotFound()
                    ...
                    .context(
                        new GhidraMcpError.ErrorContext(
                            annotation.mcpName(), "default listing", args, Map.of(), Map.of()))
                    .build());
          }
  ...
  private PaginatedResult<ListingInfo> listListingInRange(
    ...
    List<ListingInfo> allItems = new ArrayList<>();

    CodeUnit codeUnit = listing.getCodeUnitContaining(startAddr);
    if (codeUnit != null) {
      do {
        try {
          allItems.add(createListingInfo(program, codeUnit));
          ...
          && allItems.size() < maxLines * 2);
    } else {
      // If no code unit at start, try to get first code unit in range
      Address currAddr = startAddr;
      while (currAddr.compareTo(endAddr) <= 0 && allItems.size() < maxLines * 2) {
        codeUnit = listing.getCodeUnitContaining(currAddr);
        if (codeUnit != null) {
          allItems.add(createListingInfo(program, codeUnit));
          currAddr = codeUnit.getMaxAddress().add(1);
        } else {
    ...
    String finalCursorStr = cursorOpt.orElse(null);
    List<ListingInfo> paginatedItems =
        allItems.stream()
            .dropWhile(
                item -> {
                  if (finalCursorStr == null) return false;
                  return item.getAddress().compareTo(finalCursorStr) <= 0;
                })
            .limit(maxLines + 1)
            .collect(Collectors.toList());

    boolean hasMore = paginatedItems.size() > maxLines;
    List<ListingInfo> results =
        paginatedItems.subList(0, Math.min(paginatedItems.size(), maxLines));
