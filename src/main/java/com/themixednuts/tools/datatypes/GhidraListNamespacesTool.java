...
@GhidraMcpTool(name = "List Namespaces", category = ToolCategory.SYMBOLS, description = "Lists namespaces within a program, with optional filtering.", mcpName = "list_namespaces", mcpDescription = "Lists namespaces (including classes treated as namespaces), optionally filtering by path and name fragment.")
public class GhidraListNamespacesTool implements IGhidraMcpSpecification {
	...
	/**
	 * Gets available namespace names for error suggestions.
	 */
	private List<String> getAvailableNamespaces(Program program) {
		...
		while (iter.hasNext() && count < 50) { // Prevent overwhelming error messages
			...
			if (symbol.getSymbolType() == SymbolType.NAMESPACE || symbol.getSymbolType() == SymbolType.CLASS) {
				namespaces.add(symbol.getName(true));
				count++;
			}
		}
		return namespaces.stream().sorted().collect(Collectors.toList());
	}
	...
	public JsonSchema schema() {
		...
		schemaRoot.property(ARG_FILE_NAME,
				...
				.property(ARG_PATH,
						JsonSchemaBuilder.string(mapper)
								.description(
										"Optional namespace path to start listing from (e.g., 'std::my_namespace', 'MyClass'). Defaults to the global namespace."))
				.property(ARG_FILTER,
						JsonSchemaBuilder.string(mapper)
								.description("Optional case-insensitive substring filter to apply to namespace names."))
				.description("Lists namespaces under an optional path, optionally filtered by name.");
	...
	public Mono<? extends Object> execute(McpAsyncServerExchange ex, Map<String, Object> args, PluginTool tool) {
		return getProgram(args, tool)
				.flatMap(program -> {
					...
					return Mono
							.fromCallable(() -> listNamespacesAndClassesInternal(program, pathOpt, filterOpt, false));
				});
	}
	...
	private List<NamespaceInfo> listNamespacesAndClassesInternal(Program program, Optional<String> pathOpt,
		...
		if (startNamespace == null) {
			List<String> availableNamespaces = getAvailableNamespaces(program);
			...
			GhidraMcpError error = GhidraMcpError.resourceNotFound()
					...
					.context(new GhidraMcpError.ErrorContext(
							...
							Map.of("totalNamespaces", availableNamespaces.size(), "searchedPath", pathOpt.orElse("Global"))))
					.suggestions(List.of(
							new GhidraMcpError.ErrorSuggestion(
									GhidraMcpError.ErrorSuggestion.SuggestionType.CHECK_RESOURCES,
									"List available namespaces",
									"Use tools to explore available namespaces",
									null,
							...
							new GhidraMcpError.ErrorSuggestion(
									...
									"Select from available namespaces",
									availableNamespaces.isEmpty() ? List.of("Global")
											: availableNamespaces.subList(0, Math.min(10, availableNamespaces.size())),
									null)))
					.build();
					...
					if (!classesOnly || isClass) {
						if (filterLower == null || childNamespace.getName().toLowerCase().contains(filterLower)) {
							try {
								...
								System.err.println(
										"Error processing namespace: " + childNamespace.getName(true) + " - " + e.getMessage());
							}
						}
					}
				}
