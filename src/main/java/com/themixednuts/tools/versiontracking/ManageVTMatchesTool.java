...
public class ManageVTMatchesTool extends BaseMcpTool {
    ...
    public JsonSchema schema() {
        ...
        schemaRoot.property(ARG_ACTION, SchemaBuilder.string(mapper)
                .enumValues(ACTION_ACCEPT, ACTION_REJECT, ACTION_CLEAR, ACTION_ACCEPT_BULK, ACTION_REJECT_BULK)
        ...
        schemaRoot.property(ARG_SOURCE_ADDRESS, SchemaBuilder.string(mapper)
                .description("Source address of the match")
                .pattern("^(0x)?[0-9a-fA-F]+$"));

        schemaRoot.property(ARG_DESTINATION_ADDRESS, SchemaBuilder.string(mapper)
                .description("Destination address of the match")
                .pattern("^(0x)?[0-9a-fA-F]+$"));

        schemaRoot.property(ARG_MIN_SIMILARITY, SchemaBuilder.number(mapper)
                .description("Minimum similarity score for bulk operations (0.0 to 1.0)")
                .minimum(0.0)
                .maximum(1.0));

        schemaRoot.property(ARG_MIN_CONFIDENCE, SchemaBuilder.number(mapper)
                .description("Minimum confidence score for bulk operations (0.0 to 1.0)")
                .minimum(0.0)
                .maximum(1.0));

        schemaRoot.property(ARG_MAX_SIMILARITY, SchemaBuilder.number(mapper)
                .description("Maximum similarity score for bulk reject (0.0 to 1.0)")
                .minimum(0.0)
                .maximum(1.0));

        schemaRoot.property(ARG_MAX_CONFIDENCE, SchemaBuilder.number(mapper)
            ...
            try {
                return switch (normalizedAction) {
                    ...
                    default -> throw new GhidraMcpException(GhidraMcpError.invalid(ARG_ACTION, action,
                            "must be one of: " + ACTION_ACCEPT + ", " + ACTION_REJECT + ", " + ACTION_CLEAR + ", "
                            + ACTION_ACCEPT_BULK + ", " + ACTION_REJECT_BULK));
                };
            } finally {
    ...
    private Map<String, Object> handleSingleMatch(VTSession session, Map<String, Object> args,
            VTAssociationStatus newStatus) throws GhidraMcpException {
        ...
        if (targetMatch == null) {
            throw new GhidraMcpException(GhidraMcpError.notFound("match",
                    sourceAddrStr + " -> " + destAddrStr));
        }
    ...
    private Map<String, Object> handleBulkAccept(VTSession session, Map<String, Object> args)
        ...
        if (minSimilarity.isEmpty() && minConfidence.isEmpty()) {
            throw new GhidraMcpException(GhidraMcpError.validation()
                    ...
                    .message("At least one of min_similarity or min_confidence is required for bulk accept")
                    .build());
        }
    ...
    private Map<String, Object> handleBulkReject(VTSession session, Map<String, Object> args)
        ...
        if (maxSimilarity.isEmpty() && maxConfidence.isEmpty()) {
            throw new GhidraMcpException(GhidraMcpError.validation()
                    .errorCode(GhidraMcpError.ErrorCode.MISSING_REQUIRED_ARGUMENT)
                    .message("At least one of max_similarity or max_confidence is required for bulk reject")
                    .build());
        }
    ...
    private Optional<Double> getOptionalDoubleArgument(Map<String, Object> args, String argumentName) {
