...
public class ManageVTMarkupTool extends BaseMcpTool {
    ...
    public JsonSchema schema() {
        ...
        schemaRoot.property(ARG_MARKUP_TYPES, SchemaBuilder.array(mapper)
        ...
        schemaRoot.property(ARG_APPLY_ACTION, SchemaBuilder.string(mapper)
                .enumValues("REPLACE", "ADD")
                .description("How to apply markup: REPLACE (overwrite) or ADD (merge). Default: REPLACE"));
            ...
            try {
                return switch (normalizedAction) {
                    ...
                    default -> throw new GhidraMcpException(GhidraMcpError.invalid(ARG_ACTION, action,
                            "must be one of: " + ACTION_LIST + ", " + ACTION_APPLY + ", " + ACTION_APPLY_ALL + ", "
                            + ACTION_UNAPPLY));
                };
            } finally {
    ...
    private Map<String, Object> handleApply(VTSession session, Map<String, Object> args)
        ...
        if (match.getAssociation().getStatus() != VTAssociationStatus.ACCEPTED) {
            throw new GhidraMcpException(GhidraMcpError.validation()
                    ...
                    .message("Markup can only be applied to ACCEPTED matches. Current status: "
                        ...
                        for (Object item : markupItems) {
                            ...
                            try {
                                ...
                                if (failures.size() < 10) { // Limit failure messages
                                    failures.add(match.getAssociation().getSourceAddress() + " - "
                                            + getMarkupTypeName(item) + ": " + e.getMessage());
                                }
                            }
                        }
                    } catch (GhidraMcpException e) {
    ...
    private void applyMarkupItem(Object item, String applyActionStr) throws Exception {
        ...
        Class<?> actionTypeClass = Class.forName("ghidra.feature.vt.api.main.VTMarkupItemApplyActionType");
        Object applyAction = "ADD".equalsIgnoreCase(applyActionStr) ?
