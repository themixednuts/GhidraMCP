...
public class RunVTCorrelatorTool extends BaseMcpTool {
    ...
    public JsonSchema schema() {
        ...
        schemaRoot.property(ARG_SESSION_NAME, SchemaBuilder.string(mapper)
                .description("Name of the VT session"));

        schemaRoot.property(ARG_CORRELATOR_TYPE, SchemaBuilder.string(mapper)
                .enumValues("exact_bytes", "exact_instructions", "exact_data", "symbol_name")
                .description("Type of correlator to run"));

        schemaRoot.property(ARG_SOURCE_MIN_ADDRESS, SchemaBuilder.string(mapper)
                .description("Minimum address in source program to correlate")
                .pattern("^(0x)?[0-9a-fA-F]+$"));

        schemaRoot.property(ARG_SOURCE_MAX_ADDRESS, SchemaBuilder.string(mapper)
                .description("Maximum address in source program to correlate")
                .pattern("^(0x)?[0-9a-fA-F]+$"));

        schemaRoot.property(ARG_DEST_MIN_ADDRESS, SchemaBuilder.string(mapper)
                .description("Minimum address in destination program to correlate")
                .pattern("^(0x)?[0-9a-fA-F]+$"));

        schemaRoot.property(ARG_DEST_MAX_ADDRESS, SchemaBuilder.string(mapper)
                .description("Maximum address in destination program to correlate")
                .pattern("^(0x)?[0-9a-fA-F]+$"));

        schemaRoot.property(ARG_EXCLUDE_ACCEPTED, SchemaBuilder.bool(mapper)
        ...
        return Mono.fromCallable(() -> {
            ...
            return switch (normalizedAction) {
                ...
                default -> throw new GhidraMcpException(GhidraMcpError.invalid(ARG_ACTION, action,
                        "must be one of: " + ACTION_LIST + ", " + ACTION_RUN));
            };
        });
    }
    ...
    private List<VTCorrelatorInfo> handleList() {
        ...
        correlators.add(new VTCorrelatorInfo(
                "Exact Bytes Match",
                "exact_bytes",
                "Finds functions with identical byte sequences"));
        ...
        correlators.add(new VTCorrelatorInfo(
                "Exact Data Match",
                "exact_data",
                "Finds data items with identical byte values"));

        correlators.add(new VTCorrelatorInfo(
                "Symbol Name Match",
                "symbol_name",
                "Finds symbols with matching names"));
    ...
    private Map<String, Object> handleRun(Map<String, Object> args) throws GhidraMcpException {
        ...
        if (factoryClassName == null) {
            throw new GhidraMcpException(GhidraMcpError.invalid(ARG_CORRELATOR_TYPE, correlatorType,
        ...
        try {
            ...
            // Build address sets
            AddressSetView sourceSet = buildAddressSet(sourceProgram, sourceMinAddr, sourceMaxAddr,
                    session, excludeAccepted, true);
            AddressSetView destSet = buildAddressSet(destProgram, destMinAddr, destMaxAddr,
                    session, excludeAccepted, false);

            // Run correlation using reflection
            VTMatchSet matchSet = runCorrelatorReflective(session, factoryClassName,
                    sourceProgram, sourceSet, destProgram, destSet);
     ...
     */
    private VTMatchSet runCorrelatorReflective(VTSession session, String factoryClassName,
            Program sourceProgram, AddressSetView sourceSet,
            Program destProgram, AddressSetView destSet) throws GhidraMcpException {
        try {
            ...
            Method createCorrelatorMethod = factoryClass.getMethod("createCorrelator",
                    Class.forName("ghidra.framework.plugintool.ServiceProvider"),
                    Program.class, addressSetViewClass,
                    Program.class, addressSetViewClass,
                    vtOptionsClass);

            // Create correlator (null serviceProvider works for basic correlators)
            Object correlator = createCorrelatorMethod.invoke(factory,
                    null, sourceProgram, sourceSet, destProgram, destSet, options);
            ...
            try {
                ...
                Method correlateMethod = correlator.getClass().getMethod("correlate",
                        vtSessionClass, TaskMonitor.class);
    ...
    private AddressSetView buildAddressSet(Program program, Optional<String> minAddr, Optional<String> maxAddr,
            VTSession session, boolean excludeAccepted, boolean isSource) throws GhidraMcpException {
