...
public class ManageDataTypesTool extends BaseMcpTool {
                ...
                com.themixednuts.utils.jsonschema.draft7.SchemaBuilder.objectDraft7(mapper)
                        .ifThen(
                                ...
                                com.themixednuts.utils.jsonschema.draft7.SchemaBuilder.objectDraft7(mapper)
                                        ...
                                        .property(ARG_ENTRIES, SchemaBuilder.array(mapper)
                                                .items(SchemaBuilder.object(mapper)
                                                        .property(ARG_NAME,
                                                                com.themixednuts.utils.jsonschema.draft7.SchemaBuilder
                                                                        .string(mapper)
                        ...
                        .ifThen(
                                ...
                                com.themixednuts.utils.jsonschema.draft7.SchemaBuilder.objectDraft7(mapper)
                                        .property(ARG_COMMENT,
                                                com.themixednuts.utils.jsonschema.draft7.SchemaBuilder.string(mapper)
                                                        .description("Comment/description for the data type"))
                                        .property(ARG_RETURN_TYPE,
                                                com.themixednuts.utils.jsonschema.draft7.SchemaBuilder.string(mapper)
                                                        .description("Return type for function definition"))
                                        .property(ARG_PARAMETERS, SchemaBuilder.array(mapper)
                                                .items(SchemaBuilder.object(mapper)
                                                        .property(ARG_NAME,
                                                                com.themixednuts.utils.jsonschema.draft7.SchemaBuilder
                                                                        .string(mapper)
                        ...
                        .ifThen(
                                ...
                                com.themixednuts.utils.jsonschema.draft7.SchemaBuilder.objectDraft7(mapper)
                                        .property(ARG_COMMENT,
                                                com.themixednuts.utils.jsonschema.draft7.SchemaBuilder.string(mapper)
                                                        .description("Comment/description for the data type"))
                                        .not(com.themixednuts.utils.jsonschema.draft7.SchemaBuilder.objectDraft7(mapper)
        ...
        return Mono.defer(() -> {
            ...
            return executeInTransaction(program, transactionName, () -> {
                CreateDataTypeResult createResult = switch (dataTypeKind) {
                    ...
                    default ->
                        throw new IllegalArgumentException("Unsupported data type kind for creation: " + dataTypeKind);
                };
                return createResult;
            }).map(result -> {
    ...
    private CreateDataTypeResult createEnum(Map<String, Object> args, Program program, String name)
        ...
        Optional.ofNullable(entries)
                .ifPresent(entryList -> entryList.stream()
                        .filter(entry -> getOptionalStringArgument(entry, ARG_NAME).isPresent() &&
    ...
    private CreateDataTypeResult createFunctionDefinition(Map<String, Object> args, Program program, String name)
        ...
        List<Map<String, Object>> parameters = getOptionalListArgument(args, ARG_PARAMETERS).orElse(null);
        try {
                    ...
                    .ifPresent(paramList -> {
                        ...
                        List<ParameterDefinition> defs = IntStream.range(0, paramList.size())
                                .mapToObj(i -> {
                                    ...
                                    String paramName = getOptionalStringArgument(param, ARG_NAME)
                                            .orElse("param" + (i + 1));
                                    String paramType = getRequiredStringArgument(param, ARG_TYPE);
            ...
            for (Map<String, Object> member : members) {
                ...
                try {
                    if (offset == null || offset == -1) {
                        ...
                        struct.insertAtOffset(offset, memberDataType, memberDataType.getLength(), memberName,
                                memberComment);
                    }
                } catch (Exception e) {
                    throw new GhidraMcpException(
        ...
        Optional.ofNullable(parameters)
                .ifPresent(paramList -> {
                    List<ParameterDefinition> defs = IntStream.range(0, paramList.size())
                            .mapToObj(i -> {
                                Map<String, Object> param = paramList.get(i);
                                String paramTypeName = getOptionalStringArgument(param, ARG_TYPE).orElse(null);
    ...
    private OperationResult updateCategory(DataTypeManager dtm,
        ...
        if (moveOpt.isPresent()) {
            ...
            CategoryPath currentParentPath = currentParent != null ? currentParent.getCategoryPath()
                    : CategoryPath.ROOT;
    ...
    /**
     * Helper method to process struct/union members with enhanced data type
     * resolution.
    ...
    private String getDataTypeKind(DataType dataType) {
        ...
        if (typeToAnalyze instanceof Structure)
            return "struct";
        if (typeToAnalyze instanceof ghidra.program.model.data.Enum)
            return "enum";
        if (typeToAnalyze instanceof Union)
            return "union";
        if (typeToAnalyze instanceof TypeDef)
            return "typedef";
        if (typeToAnalyze instanceof Pointer)
            return "pointer";
        if (typeToAnalyze instanceof FunctionDefinitionDataType)
            return "function_definition";
        if (typeToAnalyze instanceof Array)
            return "array";
