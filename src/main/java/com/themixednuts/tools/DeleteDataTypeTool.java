...
public class DeleteDataTypeTool implements IGhidraMcpSpecification {
        ...
        public JsonSchema schema() {
                ...
                schemaRoot.property(ARG_DATA_TYPE_KIND, SchemaBuilder.string(mapper)
                                .enumValues("struct", "enum", "union", "typedef", "pointer", "function_definition",
                                                "category")
        ...
        private Mono<? extends Object> handleDelete(Program program,
                ...
                return executeInTransaction(program, "MCP - Delete " + dataTypeKind, () -> {
                        ...
                        if ("category".equalsIgnoreCase(dataTypeKind)) {
                                String name = nameOpt.orElseThrow(() -> new IllegalArgumentException(
                        ...
                        if (dataType == null) {
                                String identifier = nameOpt.orElse(dataTypeIdOpt.map(String::valueOf)
                                                .orElse("unknown"));
        ...
        private DataTypeDeleteResult deleteCategory(DataTypeManager dtm,
                ...
                if (parentCategory == null) {
                        throw new IllegalStateException(
                                        "Parent category '" + parentPath.getPath() + "' not found for '"
                                                        + targetPath.getPath() + "'");
                }
                ...
                if (!removed) {
                        GhidraMcpError error = GhidraMcpError.execution()
                                        .errorCode(GhidraMcpError.ErrorCode.TRANSACTION_FAILED)
                                        .message("Failed to delete category '" + targetPath.getPath()
                                                        + "'. Ensure it is empty.")
                                        .context(new GhidraMcpError.ErrorContext(
                                                        annotation.mcpName(),
                                                        "category deletion",
