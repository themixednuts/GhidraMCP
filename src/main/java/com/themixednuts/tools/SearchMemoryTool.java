...
@GhidraMcpTool(
    name = "Search Memory",
    description =
        "Search program memory for various data types and patterns using Ghidra's search"
            + " mechanisms",
...
public class SearchMemoryTool extends BaseMcpTool {
  ...
  public JsonSchema schema() {
    ...
    schemaRoot.property(
        ...
        SchemaBuilder.integer(mapper)
            .description(
                "Maximum number of results to return (default/max: " + DEFAULT_PAGE_LIMIT + ")")
            .minimum(1)
            ...
            program ->
                Mono.fromCallable(
                    () -> {
                      ...
                      int maxResults =
                          getOptionalIntArgument(args, ARG_MAX_RESULTS).orElse(DEFAULT_PAGE_LIMIT);
                      ...
                      SearchContext context =
                          new SearchContext(
                              program, searchType, searchValue, caseSensitive, maxResults);
  ...
  private List<SearchResult> search(SearchContext context, TaskMonitor monitor)
    ...
    if (!matcher.isValidSearch()) {
      throw new GhidraMcpException(
          GhidraMcpError.validation()
              .errorCode(GhidraMcpError.ErrorCode.INVALID_ARGUMENT_VALUE)
              .message(
                  "Invalid search value for the given search type: " + matcher.getDescription())
              .build());
    }
