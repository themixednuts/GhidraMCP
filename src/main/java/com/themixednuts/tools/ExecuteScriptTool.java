...
public class ExecuteScriptTool extends BaseMcpTool {
  ...
  public Mono<? extends Object> execute(
      McpTransportContext ex, Map<String, Object> args, PluginTool tool) {
            ...
            program ->
                Mono.fromCallable(
                    () -> {
                      ...
                      String guidanceType =
                          getOptionalStringArgument(args, ARG_GUIDANCE_TYPE).orElse("all");
  ...
  private ScriptGuidance provideScriptGuidance(
      String scriptName, String guidanceType, Program program) {
    if ("DemangleAllScript".equalsIgnoreCase(scriptName)
        || "DemangleAllScript.java".equalsIgnoreCase(scriptName)) {
  ...
  private ScriptGuidance provideDemangleAllScriptGuidance(String guidanceType, Program program) {
    ...
    return new ScriptGuidance(
        "DemangleAllScript", guidanceType, instructions, tips, troubleshooting);
  }

  private ScriptGuidance provideGeneralScriptGuidance(
      String scriptName, String guidanceType, Program program) {
    String instructions =
        """
        1. Open Ghidra and load your program
