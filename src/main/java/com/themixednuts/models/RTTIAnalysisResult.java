...
/**
 * Result of RTTI analysis using sealed interface pattern (Rust-style enum with variants).
 * Each variant (Rtti0Result, Rtti1Result, etc.) holds its specific data.
 */
public sealed interface RTTIAnalysisResult 
    ...
    enum RttiType {
        RTTI0, RTTI1, RTTI2, RTTI3, RTTI4, VFTABLE, UNKNOWN
    }
    
    // Factory methods
    static Rtti0Result from(RTTI0DataType rtti0, Program program, Address address) throws InvalidDataTypeException {
        Memory memory = program.getMemory();
        int length = rtti0.getLength(memory, address);
        Optional<String> vfTableAddress = Optional.ofNullable(rtti0.getVFTableAddress(memory, address))
            .map(Address::toString);
        Optional<String> spareDataAddress = Optional.ofNullable(rtti0.getSpareDataAddress(memory, address))
            .map(Address::toString);
    ...
    // Demangling helpers
    private static Optional<DemangledObject> tryStandardDemangler(Program program, String mangledSymbol) {
    ...
    private static Optional<DemangledObject> tryMicrosoftDemangler(Program program, String mangledSymbol, Address address) {
        if (mangledSymbol == null || mangledSymbol.trim().isEmpty()) {
            return Optional.empty();
        }
        try {
            ...
            MicrosoftMangledContext context = new MicrosoftMangledContext(program, options, mangledSymbol, address);
    ...
    record Rtti0Result(String address, Rtti0 data) implements RTTIAnalysisResult {
        @Override public boolean isValid() { return data != null; }
        @Override public RttiType rttiType() { return RttiType.RTTI0; }
    }

    record Rtti1Result(String address, Rtti1 data) implements RTTIAnalysisResult {
        @Override public boolean isValid() { return data != null; }
        @Override public RttiType rttiType() { return RttiType.RTTI1; }
    }

    record Rtti2Result(String address, Rtti2 data) implements RTTIAnalysisResult {
        @Override public boolean isValid() { return data != null; }
        @Override public RttiType rttiType() { return RttiType.RTTI2; }
    }

    record Rtti3Result(String address, Rtti3 data) implements RTTIAnalysisResult {
        @Override public boolean isValid() { return data != null; }
        @Override public RttiType rttiType() { return RttiType.RTTI3; }
    }

    record Rtti4Result(String address, Rtti4 data) implements RTTIAnalysisResult {
        @Override public boolean isValid() { return data != null; }
        @Override public RttiType rttiType() { return RttiType.RTTI4; }
    }

    record VfTableResult(String address, VfTable data) implements RTTIAnalysisResult {
        @Override public boolean isValid() { return data != null; }
        @Override public RttiType rttiType() { return RttiType.VFTABLE; }
    }

    record InvalidResult(RttiType attemptedType, String address, String error) implements RTTIAnalysisResult {
        @Override public boolean isValid() { return false; }
        @Override public RttiType rttiType() { return attemptedType; }
    }
