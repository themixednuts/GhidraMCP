...
/**
 * Provides MCP resource specifications for Ghidra data.
 * Discovers resources via ServiceLoader from META-INF/services.
 */
public class GhidraMcpResources implements IGhidraMcpResourceProvider {
    ...
    public List<AsyncResourceSpecification> getResourceSpecifications() {
        ...
        ServiceLoader.load(BaseMcpResource.class).forEach(resource -> {
            GhidraMcpResource annotation = resource.getClass().getAnnotation(GhidraMcpResource.class);
            if (annotation == null) {
                Msg.warn(this, "Resource " + resource.getClass().getSimpleName() + 
            ...
            if (!annotation.template()) {
                try {
                    ...
                    Msg.error(this, "Failed to create specification for resource: " + annotation.name(), e);
                }
            }
        });
        ...
        ServiceLoader.load(BaseMcpResource.class).forEach(resource -> {
            ...
            if (annotation.template()) {
                try {
                    ...
                    Msg.error(this, "Failed to create specification for template resource: " + annotation.name(), e);
                }
            }
        });
