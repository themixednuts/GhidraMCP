 ...
 * 
 * <p>This server is designed to be used with ApplicationLevelOnlyPlugin which ensures
 * only a single plugin instance exists. Therefore, we don't need reference counting.
 */
public final class GhidraMcpServer {
    ...
    private static final class McpSpecifications {
        ...
        boolean hasResources() { return !resources.isEmpty() || !resourceTemplates.isEmpty(); }
        boolean hasPrompts() { return !prompts.isEmpty(); }
        boolean hasCompletions() { return !completions.isEmpty(); }
    }
    ...
    private static McpSpecifications loadSpecifications(PluginTool tool) throws Exception {
        ...
        List<AsyncResourceSpecification> resources = resourceProvider != null 
            ? resourceProvider.getResourceSpecifications() : Collections.emptyList();
        List<AsyncResourceTemplateSpecification> resourceTemplates = resourceProvider != null 
            ? resourceProvider.getResourceTemplateSpecifications() : Collections.emptyList();
        if (!resources.isEmpty() || !resourceTemplates.isEmpty()) {
            Msg.info(GhidraMcpServer.class, "Loaded " + resources.size() + " resources and " + 
                    resourceTemplates.size() + " resource templates");
        }

        // Prompts (optional)
        IGhidraMcpPromptProvider promptProvider = tool.getService(IGhidraMcpPromptProvider.class);
        List<AsyncPromptSpecification> prompts = promptProvider != null 
            ? promptProvider.getPromptSpecifications() : Collections.emptyList();
        if (!prompts.isEmpty()) {
            Msg.info(GhidraMcpServer.class, "Loaded " + prompts.size() + " prompts");
        }

        // Completions (optional)
        IGhidraMcpCompletionProvider completionProvider = tool.getService(IGhidraMcpCompletionProvider.class);
        List<AsyncCompletionSpecification> completions = completionProvider != null 
            ? completionProvider.getCompletionSpecifications() : Collections.emptyList();
