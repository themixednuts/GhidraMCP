...
/**
 * Abstract base class for MCP completion providers.
 * Provides auto-completion suggestions for prompt arguments and resource template parameters.
 * 
 * <p>Implementations should be annotated with @GhidraMcpCompletion and registered
 * via ServiceLoader in META-INF/services/com.themixednuts.completions.BaseMcpCompletion.
 */
public abstract class BaseMcpCompletion {
     ...
     */
    public abstract Mono<CompleteResult> complete(McpTransportContext context, String argumentValue, PluginTool tool);
    ...
    public AsyncCompletionSpecification toCompletionSpecification(PluginTool tool) {
        return new AsyncCompletionSpecification(
                getReference(),
                (ctx, request) -> handleComplete(ctx, request, tool)
        );
    }
     ...
     */
    protected Mono<CompleteResult> handleComplete(McpTransportContext ctx, CompleteRequest request, PluginTool tool) {
        ...
        if (request.argument() == null || !getArgumentName().equals(request.argument().name())) {
            ...
            return Mono.just(new CompleteResult(
                    new CompleteResult.CompleteCompletion(List.of(), 0, false)
            ));
        }
